# 可复用 RBAC Phase 1：完整基础系统重建提示词 v1.12
...
template_inputs:
  ...
  project_db_prefix: "{{PROJECT_DB_PREFIX}}"
  container_registry: "{{CONTAINER_REGISTRY}}"
  dev_hostname: "{{DEV_HOSTNAME}}"
  ports:
    web_dev_tls: {{WEB_DEV_TLS_HOST_PORT}}
    web_redirect: {{WEB_REDIRECT_HOST_PORT}}
    web_tls: {{WEB_TLS_HOST_PORT}}
    postgres: {{POSTGRES_HOST_PORT}}
    postgres_test: {{POSTGRES_TEST_HOST_PORT}}
    redis: {{REDIS_HOST_PORT}}
    mailpit_smtp: {{MAILPIT_SMTP_HOST_PORT}}
    mailpit_ui: {{MAILPIT_UI_HOST_PORT}}
    redisinsight: {{REDISINSIGHT_HOST_PORT}}
```
...
DEV_DATABASE_NAME    = {{PROJECT_DB_PREFIX}}_dev
TEST_DATABASE_NAME   = {{PROJECT_DB_PREFIX}}_test
DATABASE_OWNER       = {{PROJECT_DB_PREFIX}}_app
RESOURCE_PREFIX      = {{PROJECT_SLUG}}-
REDIS_KEY_PREFIX     = {{PROJECT_SLUG}}:
COOKIE_NAME_PREFIX   = {{PROJECT_DB_PREFIX}}_
REFRESH_COOKIE_NAME  = {{PROJECT_DB_PREFIX}}_refresh
CSRF_COOKIE_NAME     = {{PROJECT_DB_PREFIX}}_csrf
BROWSER_KEY_PREFIX   = {{PROJECT_SLUG}}:
IMAGE_PREFIX         = {{CONTAINER_REGISTRY}}/{{PROJECT_SLUG}}-
BACKUP_EXTENSION     = .{{PROJECT_SLUG}}.backup
DEV_PUBLIC_ORIGIN    = https://{{DEV_HOSTNAME}}:{{WEB_DEV_TLS_HOST_PORT}}
```

- 根目录 `.env.example` 必须声明所有上述输入/派生变量；实际 `.env` 是 development/test 的唯一环境配置入口且不入 Git，staging/production 的秘密改由 Podman secret 或平台 secret manager 注入。`compose.yml` 顶层使用 `name: ${COMPOSE_PROJECT_NAME:?COMPOSE_PROJECT_NAME is required}`，资源引用使用 `${VARIABLE:?message}`，禁止把本次实例的名称或端口复制成 YAML/string literal。
- Context7/Compose 官方规则确认 project name 优先级为命令行 `-p`、`COMPOSE_PROJECT_NAME`、顶层 `name`、目录名。启动脚本和 CI 必须使用 `podman compose config`（provider 支持时同时检查 `config --environment`）验证最终插值值，并拒绝 `-p`/shell/`--env-file` 把 project name 覆盖为非 `{{PROJECT_SLUG}}` 的值。
...
- Refresh token 为至少 256-bit 随机值，只通过配置 `REFRESH_COOKIE_NAME`（派生值 `{{PROJECT_DB_PREFIX}}_refresh`）、属性为 `HttpOnly; Secure; SameSite=Lax; Path=/api/v1/auth` 的 Cookie 传输，数据库只存 SHA-256 hash。
- 登录同时设置配置 `CSRF_COOKIE_NAME`（派生值 `{{PROJECT_DB_PREFIX}}_csrf`）的非 HttpOnly CSRF Cookie；`refresh/logout` 必须校验精确 Origin/Referer allowlist 和 double-submit CSRF header，并与 session 中的 CSRF hash 比对。不得仅依赖 CORS。
...
对用户交付一个扩展名来自配置 `BACKUP_EXTENSION`（派生值 `.{{PROJECT_SLUG}}.backup`）的单文件 bundle，内部固定包含：
...
- 本地完整栈的唯一浏览器入口为配置 `DEV_PUBLIC_ORIGIN=https://{{DEV_HOSTNAME}}:{{WEB_DEV_TLS_HOST_PORT}}`，由 Nginx 使用本地受信 CA 证书终止 TLS；不再提供浏览器可访问的 HTTP profile 或直连 Vite host port。Nginx→Vite/API、API→本地 PostgreSQL/Redis 可在同机专用 Compose bridge 内使用明文协议，但这些内部端口不得发布到 LAN。staging 必须与 production 使用相同 TLS 强制策略。
...
|---|---:|---:|---|---|
| web/nginx dev HTTPS | 443 | `{{WEB_DEV_TLS_HOST_PORT}}` | `WEB_DEV_TLS_HOST_PORT` | 本地完整栈唯一浏览器入口；开发 CA |
| web/nginx production HTTP redirect | 80 | `{{WEB_REDIRECT_HOST_PORT}}` | `WEB_REDIRECT_HOST_PORT` | 只跳转到 HTTPS |
| web/nginx production HTTPS | 443 | `{{WEB_TLS_HOST_PORT}}` | `WEB_TLS_HOST_PORT` | 单机生产唯一业务入口 |
...
| postgres | 5432 | `{{POSTGRES_HOST_PORT}}` | `POSTGRES_HOST_PORT` | 仅开发暴露 |
| postgres-test | 5432 | `{{POSTGRES_TEST_HOST_PORT}}` | `POSTGRES_TEST_HOST_PORT` | 自动测试专用，不复用开发库 |
| redis | 6379 | `{{REDIS_HOST_PORT}}` | `REDIS_HOST_PORT` | 仅开发暴露 |
| RedisInsight | 5540 | `{{REDISINSIGHT_HOST_PORT}}` | `REDISINSIGHT_HOST_PORT` | dev-only 运维 UI；仅 loopback |
| mailpit SMTP | 1025 | `{{MAILPIT_SMTP_HOST_PORT}}` | `MAILPIT_SMTP_HOST_PORT` | 开发/测试邮件 |
| mailpit UI | 8025 | `{{MAILPIT_UI_HOST_PORT}}` | `MAILPIT_UI_HOST_PORT` | 查看验证/重置邮件 |
...
- 两个平台都必须显式使用 `--env-file .env -f deploy/compose.yml -f deploy/compose.dev.yml`，先校验配置和端口，再按依赖→migration→seed→应用的固定顺序启动。Makefile、CI 和逐条文档命令必须使用同一 env-file 与文件顺序；配置检查必须拒绝 `.env` 中不匹配 `{{PROJECT_SLUG}}` 的 project identity。任一步失败都输出失败阶段和下一条安全排障命令，不能继续启动一个半配置状态的应用栈。
...
- development profile 的 RedisInsight 使用锁定 digest、独立命名 volume 和 `/api/health/`，仅绑定 `${DEV_BIND_ADDRESS}:{{REDISINSIGHT_HOST_PORT}}` 并能通过 `redis:6379` 连接本项目 Redis；staging/production 配置、镜像清单和 volume 中不存在 RedisInsight。
